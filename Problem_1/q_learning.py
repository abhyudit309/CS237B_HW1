...
from utils import map_chunked, generate_problem, visualize_value_function, simulate_MDP
...
def Q_learning(Q_network, reward_fn, is_terminal_fn, X, U, Xp, gam):
    ...
    def loss():
        ...
        # elements of mask are 0 if terminal state, otherwise 1
        mask = tf.cast(tf.cast(is_terminal_fn(X_), tf.int32) == 0, tf.float32)
        temporal_diff = reward_fn(X_, U_) + gam * mask * next_Q - Q
        l = tf.reduce_mean(temporal_diff ** 2)
    ...
    for _ in tqdm(range(int(1e4))):
        ...
        optimizer.minimize(loss, Q_network.variables)
...
def main():
    ...
    Q_network = tf.keras.Model(inputs, output, name='Q_network')
    ...
    plt.figure(120)
    opt_policy = visualize_value_function(V.numpy().reshape((n, n)))
    ...
    # visualize the result
    plt.figure(120)
    simulate_MDP(V.numpy().reshape((n, n)), Ts)
    plt.colorbar()
    plt.title("MDP simulation - Q learning")
    plt.show()
