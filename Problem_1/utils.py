...
def visualize_value_function(V):
    ...
    visualized as an arrow. Also, returns the optimal policy.
    ...
    u, v = [], []
    opt_policy = -1 * np.ones((m, n), dtype=int)
    for pt in pts:
...
def simulate_MDP(V, Ts):
    """
    Simulates the Markov Decision Process given the value function V
    and list of state transition matrices Ts.

    Args:
        V: (np.array) the value function reshaped into a 2D array.
        Ts: list of state transition matrices.
    """
    opt_policy = visualize_value_function(V)
    m, n = V.shape
    ...
    pt = x_start
    traj_pts.append(pt)
    while not np.array_equal(pt, x_goal):
        opt_action = opt_policy[pt[0], pt[1]]
        transition_mat = Ts[opt_action]
        prob = transition_mat[pos2idx[pt[0], pt[1]]]
        next_idx = tf.random.categorical(tf.math.log([prob]), 1).numpy()[0, 0]
        next_point = idx2pos[next_idx]

        u.append(next_point[0] - pt[0])
        v.append(next_point[1] - pt[1])
        traj_pts.append(next_point)
        pt = next_point

    traj_pts = np.array(traj_pts)
    plt.quiver(traj_pts[:-1, 0], traj_pts[:-1, 1], u, v, pivot="middle", color='red')
