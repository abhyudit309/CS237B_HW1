...
def get_bottleneck_dataset(model, img_dir, img_size):
    ...
    for i in range(train_img_gen.samples):
        ...
        bottleneck_output = model(x_i)
        bottleneck_x_l.append(bottleneck_output)
...
def retrain(image_dir):
    ...
    retrain_input = tf.keras.Input(shape=base_model.output.get_shape().as_list()[1])
    retrain_layer = tf.keras.layers.Dense(3, activation='softmax', name='classifier')
    retrain_output = retrain_layer(retrain_input)
    retrain_model = tf.keras.Model(retrain_input, retrain_output)
    ...
    model = tf.keras.models.Sequential([base_model, retrain_layer])
