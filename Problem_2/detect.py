...
def compute_brute_force_classification(model, image_path, nH=8, nW=8):
    ...
    for i in range(nH):
        for j in range(nW):
            # x and y indices
            x_ind = np.arange(i * Lx - pad, i * Lx + Lx + pad)
            y_ind = np.arange(j * Ly - pad, j * Ly + Ly + pad)
            # ensuring that the padded window is within the image
            x_ind = x_ind[np.logical_and(x_ind >= 0, x_ind <= H-1)]
            y_ind = y_ind[np.logical_and(y_ind >= 0, y_ind <= W-1)]
            window = raw_image[x_ind[0]:x_ind[-1]+1, y_ind[0]:y_ind[-1]+1, :]
            normalized_window = normalize_resize_image(window, IMG_SIZE)
            window_model = np.expand_dims(normalized_window, axis=0)
            pred = model(window_model)
            window_predictions[i, j, :] = pred.numpy()[0]
...
def compute_convolutional_KxK_classification(model, image_path):
    ...
    # First calculate K
    K = conv_model.layers[-1].output.shape[1]
    L = conv_model.layers[-1].output.shape[-1]

    # Next create a intermediate input structure which takes in a bottleneck tensor
    int_input = tf.keras.Input(shape=L)
    ...
    linear_classifier = model.get_layer('classifier')
    output = linear_classifier(int_input)
    classifier_model = tf.keras.Model(int_input, output, name='classifier model')

    # Predict the ouput of the convolution layer using conv_model
    image = np.expand_dims(resized_patch, axis=0)
    conv_output = conv_model(image)

    # Reshape so that patches become batches and predict
    conv_output = tf.reshape(conv_output, (K*K, L))
    predictionsKxK = classifier_model(conv_output)
...
def compute_and_plot_saliency(model, image_path):
    ...
    with tf.GradientTape() as t:
        ...
        image = tf.expand_dims(normalize_resize_image(raw_image, IMG_SIZE), axis=0)
        t.watch(image)
        S = logits_model(image)[0]
        top_class = tf.argmax(S)
        Sc = S[top_class]
        w = t.gradient(Sc, image)
        M = tf.reduce_max(tf.abs(w), axis=-1)[0]
